...
class McpToolRegistryTest {
    ...
    @Test
    fun `ADB_REQUIRED_TOOLS is a subset of all tools`() {
        val allNames = tools.map { it.name }.toSet()
        McpToolRegistry.ADB_REQUIRED_TOOLS.forEach { adbTool ->
            assertTrue("ADB required tool '$adbTool' not in registry", adbTool in allNames)
        }
    }
